class CarRepository:
    ...
    def get_by_id(self, car_id):
        try:
            result = self.db_manager.execute_query(
                "SELECT * FROM lyfter_car_rental.cars WHERE id = %s;", car_id
            )
            if result:
    ...
    def update_car(self, _id, model_id, year, state_id):
        try:
            self.db_manager.execute_query(
                """
                UPDATE lyfter_car_rental.cars SET (model_id, year, state_id) = (%s, %s, %s) WHERE id = %s
                """,
                (model_id, year, state_id, _id),
            )
    ...
    def update_state(self, car_id, state_id):
        try:
            self.db_manager.execute_query(
                """
                UPDATE lyfter_car_rental.cars SET state_id = %s WHERE id = %s
                """,
                (state_id, car_id),
            )
    ...
    def disable_car(self, car_id):
        ...
        try:
            self.db_manager.execute_query(
                """
                UPDATE lyfter_car_rental.cars SET state_id = 2 WHERE id = %s
                """,
                (car_id,),
            )
    ...
    def get_rented_cars(self):
        ...
        try:
            results = self.db_manager.execute_query(
                "SELECT * FROM lyfter_car_rental.cars WHERE state_id = 3;"
            )
            formatted_results = [self._format_car(result) for result in results]
            return formatted_results
        except Exception as error:
    ...
    def get_available_cars(self):
        ...
        try:
            results = self.db_manager.execute_query(
                "SELECT * FROM lyfter_car_rental.cars WHERE state_id = 1;"
            )
            formatted_results = [self._format_car(result) for result in results]
            return formatted_results
        except Exception as error:
