class RentalRepository:
    ...
    def get_by_id(self, rental_id):
        try:
            result = self.db_manager.execute_query(
                "SELECT * FROM lyfter_car_rental.rentals WHERE id = %s;", rental_id
            )
            if result:
    ...
    def insert_new_rental(self, user_id, car_id):
        try:
            self.db_manager.execute_query(
                ...
                (%s, %s, %s)
                """,
                (user_id, car_id, 1),  # state_id=1 for active
            )
        except Exception as error:
    ...
    def update_rental(self, _id, user_id, car_id, state_id):
        try:
            self.db_manager.execute_query(
                """
                UPDATE lyfter_car_rental.rentals SET (user_id, car_id, state_id) = (%s, %s, %s) WHERE id = %s
                """,
                (user_id, car_id, state_id, _id),
            )
    ...
    def complete_rental(self, rental_id, car_repository):
        try:
            ...
            self.db_manager.execute_query(
                """
                UPDATE lyfter_car_rental.rentals SET state_id = 2 WHERE id = %s
                """,
                (rental_id),
            )
            ...
            result = self.db_manager.execute_query(
                "SELECT car_id FROM lyfter_car_rental.rentals WHERE id = %s;", rental_id
            )
            if not result:
